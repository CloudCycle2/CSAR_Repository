...
public class UpdateOpenToscaServerServlet extends AbstractServlet {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(UpdateOpenToscaServerServlet.class);
	...
	protected void doPost(HttpServletRequest request,
		...
		Long openToscaServerId = Long.parseLong(request
				.getParameter(PARAM_OPEN_TOSCA_SERVER_ID));
		String openToscaServerName = request
				.getParameter(PARAM_OPEN_TOSCA_SERVER_NAME);
		String OpenToscaServerAddress = request
				.getParameter(PARAM_OPEN_TOSCA_SERVER_URL);

		UpdateOpenToscaServerService service = new UpdateOpenToscaServerService(
				0L, openToscaServerId, openToscaServerName,
				OpenToscaServerAddress);

		LOGGER.debug("Request to update open tosca server " + openToscaServerId
				+ " handeled by servlet");
		...
		this.redirect(
				request,
				response,
				OpenToscaServerDetailsServlet.PATH.replaceFirst("\\*",
						Long.toString(openToscaServerId)));
